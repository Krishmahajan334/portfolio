...
${data.message}
        ...
        const mailtoLink = `mailto:krishmahajan334@gmail.com?subject=${emailSubject}&body=${emailBody}`;
      ...
      if (userResponse.ok && reposResponse.ok) {
        ...
        try {
          ...
          if (commitsResponse.ok) {
            ...
            if (commitEl) commitEl.textContent = commitsData.total_count || '0';
          } else {
            const commitEl = document.getElementById('githubCommits');
            if (commitEl) commitEl.textContent = '10+'; // Fallback if search API fails
          }
        } catch (commitErr) {
          console.error('Error fetching commits:', commitErr);
          const commitEl = document.getElementById('githubCommits');
          if (commitEl) commitEl.textContent = '5+';
        }
